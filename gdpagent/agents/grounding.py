"""
This is adopted from Agent S2.5.
"""
import ast
import json
import re
...
from gdpagent.s2_5.memory.procedural_memory import PROCEDURAL_MEMORY
from gdpagent.s2_5.core.mllm import LMMAgent
from gdpagent.s2_5.utils.common_utils import (
    call_llm_safe,
...
class OSWorldACI(ACI):
    def __init__(
        ...
        """
        Initialize OSWorldACI with built-in and optional custom actions.
        
        Args:
            platform: OS platform (linux, darwin, windows)
            engine_params_for_generation: Parameters for the generation LLM
            engine_params_for_grounding: Parameters for the grounding model
            width: Screen width in pixels
            height: Screen height in pixels
            custom_actions: Optional list of custom action functions to register.
                Each function should be decorated with @agent_action or have is_agent_action=True attribute.
                Functions will be dynamically bound to this instance.
        """
        self.platform = (
    ...
    def _register_custom_actions(self, custom_actions: List):
        """
        Register custom action functions to this agent instance.
        
        This method dynamically binds custom functions to the agent, making them available
        for the LLM to call. Each function should:
        1. Have the @agent_action decorator (or is_agent_action=True attribute)
        2. Include a docstring describing its purpose and parameters (used by LLM)
        3. Return executable Python code as a string (like built-in actions)
        
        Args:
            custom_actions: List of functions to register as agent actions
        
        Example:
            @agent_action
            def my_custom_action(self, param1: str, param2: int = 5):
                '''Custom action description
                Args:
                    param1: str, description of param1
                    param2: int, description of param2
                '''
                return f"import my_module; my_module.do_something('{param1}', {param2})"
            
            agent = OSWorldACI(..., custom_actions=[my_custom_action])
        """
        import types
        
        for action_func in custom_actions:
            # Verify the function has the agent_action marker
            if not hasattr(action_func, 'is_agent_action'):
                # Auto-mark it if not already marked
                action_func.is_agent_action = True
            
            # Bind the function to this instance as a method
            func_name = action_func.__name__
            bound_method = types.MethodType(action_func, self)
            setattr(self, func_name, bound_method)
            
            print(f"Registered custom action: {func_name}")
    ...
    def resize_coordinates(self, coordinates: List[int]) -> List[int]:
        ...
        grounding_width = int(float(grounding_width))
        grounding_height = int(float(grounding_height))
...
class GroundingActionExecutor:
    """Execute GUI actions using OSWorldACI grounding and the DesktopEnv."""
    ...
    def _get_obs(self) -> Dict[str, Any]:
        try:
            ...
            return {"screenshot": self.env.controller.get_screenshot()}

    def _reset_coords(self) -> None:
        self.aci_agent.coords1 = None
        self.aci_agent.coords2 = None

    def run_action(self, action_name: str, **kwargs: Any) -> str:
        self._reset_coords()
        obs = self._get_obs()

        if action_name in {"click", "type", "scroll"}:
            element_description = kwargs.get("element_description")
            if element_description:
                self.aci_agent.coords1 = self.aci_agent.generate_coords(element_description, obs)
        elif action_name == "drag_and_drop":
            starting_description = kwargs.get("starting_description")
            ending_description = kwargs.get("ending_description")
            if starting_description:
                self.aci_agent.coords1 = self.aci_agent.generate_coords(starting_description, obs)
            if ending_description:
                self.aci_agent.coords2 = self.aci_agent.generate_coords(ending_description, obs)

        action_fn = getattr(self.aci_agent, action_name, None)
        if action_fn is None:
            raise ValueError(f"Unknown ACI action '{action_name}'")

        command = action_fn(**kwargs)
        observation, *_ = self.env.step(command, self.sleep_after_execution)
        out = observation.get("action_output", None)
        if isinstance(out, dict):
            return json.dumps(out, ensure_ascii=False)
        return str(out) if out else "Action executed."
