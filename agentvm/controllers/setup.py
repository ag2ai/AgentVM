...
from agentvm.evaluators.metrics.utils import compare_urls
from agentvm.providers.aws.proxy_pool import get_global_proxy_pool, init_proxy_pool, ProxyInfo
...
PROXY_CONFIG_FILE = os.getenv("PROXY_CONFIG_FILE", "agentvm/dataimpulse.json")  # Default proxy config file
...
class SetupController:
    ...
    def setup(self, config: List[Dict[str, Any]], use_proxy: bool = False)-> bool:
        ...
        for i, cfg in enumerate(config):
            ...
            try:
                ...
                logger.error("An error occurred while running %s: %s", setup_function, e)
                traceback.print_exc()
                return False

        return True

    def _download_file_setup(self, url: str, path: str):
        if not url or not path:
            raise Exception(f"Setup Download - Invalid url or path ({url}, {path}).")
        ...
        try:
            response = requests.post(self.http_server + "/setup" + "/download_file", headers=headers, data=payload)
            if response.status_code == 200:
                logger.info("File downloaded successfully: %s", response.text)
            else:
                logger.error("Failed to download file. Status code: %s", response.text)
    ...
    def _execute_setup(
        ...
        def replace_screen_env_in_command(command: Union[str, List[str]]):
        ...
        def normalize_command(command: Union[str, List[str]], shell: bool) -> Union[str, List[str]]:
            """Normalize command for remote execution.

            Common pitfall: passing ["pip3 install ..."] with shell=False makes subprocess try to
            find an executable literally named "pip3 install ...".
            """
            if shell:
                return command
            ...
            # If user passed a one-element argv that still contains spaces, split it.
            if len(command) == 1 and isinstance(command[0], str) and any(ch.isspace() for ch in command[0]):
                logger.warning(
                    "Command looks like a single string with spaces while shell=False; splitting it. "
                    "Consider passing a list of args or set shell=True. Command=%r",
                    command[0],
                )
        ...
        command = normalize_command(command, shell)
        payload_dict = {"command": command, "shell": shell}
        if timeout is not None:
            payload_dict["timeout"] = timeout
            # logger.info(f"[_execute_setup] Setting timeout in payload: {timeout} seconds")
        # else:
        #     logger.info("[_execute_setup] No timeout specified in payload (will use server default)")
        payload = json.dumps(payload_dict)
        headers = {"Content-Type": "application/json"}
        # logger.info(f"[_execute_setup] Request timeout parameter: {timeout}")
        # logger.info(f"[_execute_setup] Payload: {payload}")
    ...
    def _register_action_setup(self, actions: List[Dict[str, Any]]):
        """
        Set up actions bundles in the Docker container at /home/user/actions.
        
        Args:
            actions (List[Dict[str, Any]]): List of actions bundle configurations. Each dict has:
                {
                    "local_path": str,     # Local path to the bundle directory
                    "config": Dict[str, Any],  # action configuration (passed as-is to remote)
                    "force_reinstall": bool,   # (optional) if True, reinstall even if already installed
                }
        """
        if not actions:
            logger.info("No actions to install")
            return
        
        logger.info("Starting actions setup for %d actions", len(actions))
        
        import tarfile
        
        actions_dir = "/home/user/actions"
        allowed_actions_file = os.path.join(actions_dir, ".allowed_actions.txt")
        tmp_files_to_cleanup = []
        
        try:
            # Step 0: Read installed actions from remote
            logger.info("Checking for previously installed actions")
            installed_actions = set()
            try:
                # Read the allowed actions file from remote VM
                self._execute_setup([f"cat {allowed_actions_file}"], shell=True, stdout="allowed_actions.txt")
                allowed_file_path = os.path.join(self.cache_dir, "allowed_actions.txt")
                if os.path.exists(allowed_file_path):
                    with open(allowed_file_path, 'r') as f:
                        for line in f:
                            action_name = line.strip()
                            if action_name:
                                installed_actions.add(action_name)
                    logger.info(f"Found {len(installed_actions)} previously installed actions: {installed_actions}")
            except Exception as e:
                logger.debug(f"No previous .allowed_actions.txt file found or unable to read: {e}")
            
            # Step 1: Determine which actions to install and which configs to upload
            logger.info("Filtering actions and preparing configs")
            actions_to_install = []
            configs_to_upload = {}  # Maps action_name -> (local_path, temp_config_path)
            
            for action in actions:
                action_name = os.path.basename(action["local_path"].rstrip("/"))
                force_reinstall = action.get("force_reinstall", False)
                local_config_path = os.path.join(action["local_path"], "config.json")
                
                # Determine if action needs installation
                needs_install = force_reinstall or action_name not in installed_actions
                
                # Always prepare config for all actions (whether installing or not)
                config = {}
                if os.path.exists(local_config_path):
                    with open(local_config_path, 'r') as f:
                        config = json.load(f)
                
                passed_config = action.get("config", {})
                if passed_config:
                    config.update(passed_config)
                
                # Create temp config file if there's any config to upload
                if config:
                    tmp_config = tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False)
                    json.dump(config, tmp_config, indent=2)
                    tmp_config.close()
                    tmp_files_to_cleanup.append(tmp_config.name)
                    configs_to_upload[action_name] = (action["local_path"], tmp_config.name)
                    logger.info(f"Prepared config for {action_name}")
                
                # Log status and add to install list if needed
                if needs_install:
                    actions_to_install.append(action)
                else:
                    logger.info(f"Skipping install for {action_name} (already installed)")
            
            # Step 2: Validate paths and locate helper script (only if installing)
            if actions_to_install:
                # logger.info("Validating %d action bundles", len(actions_to_install))
                for action in actions_to_install:
                    if not os.path.exists(action["local_path"]):
                        raise Exception(f"Bundle path not found: {action['local_path']}")
                
                action_setup_script = os.path.join(os.path.dirname(os.path.dirname(__file__)), "..", "actions", "action_setup.sh")
                if not os.path.exists(action_setup_script):
                    raise Exception(f"Helper file not found: {action_setup_script}")
            
            # Step 3: Create tar archive with actions to install and/or configs to upload
            if not actions_to_install and not configs_to_upload:
                logger.info("No actions to install and no configs to upload, skipping")
                return
            
            # logger.info("Creating tar archive with %d actions to install and %d config files", len(actions_to_install), len(configs_to_upload))
            tmp_tar = tempfile.NamedTemporaryFile(suffix=".tar.gz", delete=False)
            tar_path = tmp_tar.name
            tmp_tar.close()
            tmp_files_to_cleanup.append(tar_path)
            
            def tar_filter(tarinfo):
                if '/.venv/' in tarinfo.name or tarinfo.name.endswith('/.venv') or '/venv/' in tarinfo.name or tarinfo.name.endswith('/venv'):
                    return None
                if tarinfo.name.endswith('/state.json') or tarinfo.name.endswith('\\state.json'):
                    return None
                return tarinfo
            
            with tarfile.open(tar_path, "w:gz") as tar:
                # Add setup script only if we're installing actions
                if actions_to_install:
                    tar.add(action_setup_script, arcname="actions/action_setup.sh")
                    # Add only actions that need installation
                    for action in actions_to_install:
                        action_name = os.path.basename(action["local_path"].rstrip("/"))
                        tar.add(action["local_path"], arcname=f"actions/{action_name}", filter=tar_filter)
                
                # Add all config files (for both new installs and existing actions)
                for action_name, (local_path, config_path) in configs_to_upload.items():
                    tar.add(config_path, arcname=f"actions/{action_name}/config.json")

            logger.info("Created archive: %.2f MB", os.path.getsize(tar_path) / (1024 * 1024))
            
            # Step 4: Upload archive
            form = MultipartEncoder({
                "file_path": "/tmp/actions.tar.gz",
                "file_data": ("actions.tar.gz", open(tar_path, "rb"))
            })
            response = requests.post(
                self.http_server + "/setup/upload",
                headers={"Content-Type": form.content_type},
            ...
            if response.status_code != 200:
                raise Exception(f"Failed to upload actions archive: {response.text}")
        ...
        logger.info("Extracting actions archive")
        self._execute_setup([f"mkdir -p {actions_dir}"], shell=True)
        self._execute_setup([f"tar -xzf /tmp/actions.tar.gz -C /home/user && rm /tmp/actions.tar.gz"], shell=True, timeout=300)
        
        # Step 6: Run setup script only if there are actions to install
        if actions_to_install:
            logger.info("Running setup script for %d actions", len(actions_to_install))
            # Build command to run setup script with action names to install
            setup_cmd = f"bash {actions_dir}/action_setup.sh"
            for action in actions_to_install:
                action_name = os.path.basename(action["local_path"].rstrip("/"))
                setup_cmd += f" {action_name}"
            
            start_time = time.time()
            self._execute_setup([setup_cmd], shell=True, timeout=600)
            elapsed = (time.time() - start_time) / 60
            logger.info(f"Setup completed in {elapsed:.2f} minutes")
        else:
            logger.info("No actions to install, skipping setup script execution. Config files uploaded successfully.")
