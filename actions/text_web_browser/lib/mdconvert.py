...
class DocumentConverterResult:
    ...
    def __init__(self, title: Union[str, None] = None, text_content: str = "", converter_name: str = ""):
        self.title: Union[str, None] = title
        self.text_content: str = text_content
        self.converter_name: str = converter_name
...
class HtmlConverter(DocumentConverter):
    ...
    def _convert(self, html_content: str) -> Union[None, DocumentConverterResult]:
        ...
        return DocumentConverterResult(
            title=None if soup.title is None else soup.title.string, text_content=webpage_text, converter_name="HtmlConverter"
        )
...
class PdfConverter(DocumentConverter):
    ...
    #TODO: check out other potential options. https://github.com/VikParuchuri/marker?tab=readme-ov-file or mistral ocr
    ...
    def convert(self, local_path, **kwargs) -> Union[None, DocumentConverterResult]:
        ...
        # get the file name
        local_save_path = "converted_pdfs"
        file_name = os.path.basename(local_path)
        converted_file_name = f"{local_save_path}/{file_name.split('.')[0]}"

        if not os.path.exists(converted_file_name+".html") and not os.path.exists(converted_file_name+".md"):
            # TODO: return pdfminer for now
            return DocumentConverterResult(
                title=None,
            ...
            print(f"Converting {file_name} to HTML with Chunkr")
            chunkr = Chunkr(api_key=os.environ["CHUNKR_API_KEY"])
            task = chunkr.upload(local_path)
            print("Uploading Complete")
            task.html(output_file=converted_file_name)
            chunkr.close()
            print(f"Saved {file_name} as {file_name.split('.')[0]}.html")

        # local_save_path = "converted_pdfs"
        # for converted_file in os.listdir(local_save_path):
        #     if file_name.split('.')[0] == converted_file.split('.')[0]:
        #         return html_converter.convert(f"{local_save_path}/{converted_file}")
        if os.path.exists(converted_file_name+".html"):
            html_converter = HtmlConverter()
            return html_converter.convert(converted_file_name+".html", file_extension=".html")
        else:
            document_converter = PlainTextConverter()
            return document_converter.convert(converted_file_name+".md", file_extension=".md")
...
class XlsxConverter(HtmlConverter):
    ...
    def convert(self, local_path, **kwargs) -> Union[None, DocumentConverterResult]:
        # Bail if not XLS/XLSX
        extension = kwargs.get("file_extension", "")
        if extension.lower() not in [".xlsx", ".xls"]:
            return None

        # Load the workbook using openpyxl with data_only=True to get cell values
        ...
        for sheet_name in wb.sheetnames:
            ...
            for row in ws.iter_rows():
                html_table += "  <tr>\n"
                for cell in row:
                    # Extract cell text (or an empty string if None)
                    cell_text = str(cell.value) if cell.value is not None else ""

                    # Default to no inline background style
                    style_attr = ""

                    # If fill is solid, try to get the color
                    if cell.fill and cell.fill.fill_type == "solid":
                        fill_color = cell.fill.start_color
                        if fill_color is not None:
                            # Check if color type is 'rgb' (versus 'theme', 'indexed', etc.)
                            if fill_color.type == "rgb" and fill_color.rgb is not None:
                                # fill_color.rgb is often a string like 'FF00FF00' (ARGB)
                                rgb_str = fill_color.rgb  # e.g. 'FF00FF00'
                                # Ensure it's actually a string
                                if isinstance(rgb_str, str):
                                    # If 8 chars, likely ARGB; remove the first two (alpha)
                                    if len(rgb_str) == 8:
                                        # e.g. 'FF00FF00' -> '00FF00'
                                        rgb_str = rgb_str[-6:]
                                    style_attr = f' style="background-color: #{rgb_str};"'

                    # Build the cell HTML
                    html_table += f'    <td{style_attr}>{cell_text}</td>\n'
                html_table += "  </tr>\n"
            html_table += "</table>\n\n"

            # Append the table for this sheet to the markdown content
            md_content += html_table
...
class WavConverter(MediaConverter):
    ...
    def _transcribe_audio(self, local_path) -> str:
        ...
        with sr.AudioFile(local_path) as source:
            ...
            return recognizer.recognize_whisper(audio, model="turbo").strip()
...
class ZipConverter(DocumentConverter):
    ...
    def convert(self, local_path: str, **kwargs: Any) -> Union[None, DocumentConverterResult]:
        ...
        return DocumentConverterResult(title="Extracted Files", text_content=md_content.strip(), converter_name="ZipConverter")
...
class ImageConverter(MediaConverter):
    ...
    def convert(self, local_path, **kwargs) -> Union[None, DocumentConverterResult]:
        ...
        mlm_model = kwargs.get("mlm_model", "gpt-4o")
